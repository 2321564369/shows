...
function doPoll(event, news_id) {
    ...
    $.post(dle_root + "engine/ajax/controller.php?mod=poll", {
        news_id: news_id, action: event, answer: vote_check, dle_skin: dle_skin, user_hash: dle_login_hash
    }, function (data) {
...
function ajax_prep_for_edit(news_id, event) {
    ...
    $.get(dle_root + "engine/ajax/controller.php?mod=editnews", {
        id: news_id, field: event, action: "edit"
    }, function (data) {
...
function ajax_save_comm_edit(c_id, area) {
    ...
    $.post(dle_root + "engine/ajax/controller.php?mod=editcomments", {
        id: c_id, comm_txt: comm_txt, area: area, action: "save", user_hash: dle_login_hash
    }, function (data) {
...
function MarkSpam(id, hash) {
    ...
    DLEconfirm(dle_spam_agree, dle_confirm, function () {
        ...
        $.get(dle_root + "engine/ajax/controller.php?mod=adminfunction", {
            id: id, action: 'commentsspam', user_hash: hash
        }, function (data) {
...
function doFavorites(fav_id, event, alert) {
    ...
    $.get(dle_root + "engine/ajax/controller.php?mod=favorites", {
        fav_id: fav_id, action: event, skin: dle_skin, alert: alert, user_hash: dle_login_hash
    }, function (data) {
...
function CheckLogin() {
    ...
    $.post(dle_root + "engine/ajax/controller.php?mod=registration", {
        name: name, user_hash: dle_login_hash
    }, function (data) {
...
function doRate(rate, id) {
    ...
    $.get(dle_root + "engine/ajax/controller.php?mod=rating", {
        go_rate: rate, news_id: id, skin: dle_skin, user_hash: dle_login_hash
    }, function (data) {
        ...
        if (data.success) {
            ...
            var ratingscore = data.ratingscore;
            var voteNum = data.votenum;

            rating = rating.replace(/&lt;/g, "<");
            rating = rating.replace(/&gt;/g, ">");
            rating = rating.replace(/&amp;/g, "&");

            function getVoteText(num) {
            ...
            var voteText = getVoteText(voteNum);

            $('[data-rating-score-id="' + id + '"]').html(ratingscore);
            $('[data-rating-layer-id="' + id + '"]').html(rating);
            $('[data-vote-num-id="' + id + '"]').html(voteNum + " " + voteText);
...
function doCommentsRate(rate, id) {
    ...
    $.get(dle_root + "engine/ajax/controller.php?mod=ratingcomments", {
        go_rate: rate, c_id: id, skin: dle_skin, user_hash: dle_login_hash
    }, function (data) {
...
function DLESendPM(name) {
    ...
    $.get(dle_root + "engine/ajax/controller.php?mod=pm", {
        name: name, action: 'show_send', skin: dle_skin, user_hash: dle_login_hash
    }, function (data) {
...
function dle_reply(id, indent, simple) {
    ...
    $.get(dle_root + "engine/ajax/controller.php?mod=replycomments", {
        id: id, indent: indent, skin: dle_skin, user_hash: dle_login_hash, needwrap: needwrap
    }, function (data) {
            ...
            bt[dle_p_send] = function () {
                if ($('#dle-promt-text').val().length < 1) {
                    ...
                    $.post(dle_root + 'engine/ajax/controller.php?mod=message', {
                        id: id, user_hash: dle_login_hash, text: response
                    }, function (data) {
                        if (data == 'ok') {
                            document.location = dle_root + 'index.php?do=deletenews&id=' + id + '&hash=' + dle_login_hash;
                        } else {
                            DLEalert('Send Error', dle_info);
                        }
                    });
...
function sendNotice(id) {
    ...
    b[dle_p_send] = function () {
        if ($('#dle-promt-text').val().length < 1) {
            ...
            $('#dlepopup').remove();
            $.post(dle_root + 'engine/ajax/controller.php?mod=message', {
                id: id, user_hash: dle_login_hash, text: response, allowdelete: "no"
            }, function (data) {
                if (data == 'ok') {
...
function DLEalert(message, title) {
    ...
    $('#dlepopup').dialog({
        modal: true, autoOpen: true, width: 470, resizable: false, dialogClass: "modalfixed dle-popup-alert", buttons: {
            "Ok": function () {
                $(this).dialog("close");
                $("#dlepopup").remove();
            }
...
function DLEprompt(message, d, title, callback, allowempty, type = 'text') {
    ...
    $('#dlepopup').dialog({
        modal: true, autoOpen: true, width: 500, resizable: false, dialogClass: "modalfixed dle-popup-promt", buttons: b
    });
...
function ShowPopupProfile(r, allowedit) {
    ...
    $('#dleprofilepopup').dialog({
        modal: true, autoOpen: true, resizable: false, dialogClass: "dle-popup-userprofile", buttons: b, width: 550
    });
...
function onTwofactoryChange(obj, allowchange) {
    ...
    if (value && value == 2 && value != prev_value) {
        ...
        $.get(dle_root + "engine/ajax/controller.php?mod=twofactor", {
            mode: 'createsecret', skin: dle_skin, user_hash: dle_login_hash
        }, function (data) {
            ...
            b[dle_act_lang[2]] = function () {
                if ($("#dle-promt-text").val().length < 1) {
                    ...
                    $.post(dle_root + "engine/ajax/controller.php?mod=twofactor", {
                        mode: 'verifysecret', pin: pin, skin: dle_skin, user_hash: dle_login_hash
                    }, function (data) {
...
function ShowProfile(name, url, allowedit) {
    ...
    if (dle_user_profile == name && document.getElementById('dleprofilepopup')) {
    ...
    $.get(dle_root + "engine/ajax/controller.php?mod=profile", {
        name: name, skin: dle_skin, user_hash: dle_login_hash
    }, function (data) {
...
function dle_do_search(inputString) {
    ...
    $("#search-ajax-result").append("<div id='searchsuggestions' style='display:none'></div>");

    $.post(dle_root + "engine/ajax/controller.php?mod=search", {
        query: "" + inputString + "", skin: dle_skin, user_hash: dle_login_hash
    }, function (data) {

        $('#searchsuggestions').html(data).fadeIn().css({'position': 'absolute', top: 0, left: 0}).position({
            my: "left top", at: "left bottom", of: "#story"
        });
...
function ShowLoading(message, positionx, positiony) {
    ...
    $("#loading-layer").css({
        left: setX + "px", top: setY + "px", position: 'fixed', zIndex: '9999'
    });
...
function ShowAllVotes() {
    ...
    $.get(dle_root + "engine/ajax/controller.php?mod=allvotes&dle_skin=" + dle_skin, function (data) {
        ...
        $('#dlevotespopup').dialog({
            modal: true, autoOpen: true, resizable: false, dialogClass: "dle-popup-allvotes", width: 600
        });
...
function AddIgnorePM(id, text) {
    ...
    DLEconfirm(text, dle_confirm, function () {
        ...
        $.get(dle_root + "engine/ajax/controller.php?mod=adminfunction", {
            id: id, action: "add_ignore", skin: dle_skin, user_hash: dle_login_hash
        }, function (data) {
...
function DelIgnorePM(id, text) {
    ...
    DLEconfirm(text, dle_confirm, function () {
        ...
        $.get(dle_root + "engine/ajax/controller.php?mod=adminfunction", {
            id: id, action: "del_ignore", skin: dle_skin, user_hash: dle_login_hash
        }, function (data) {
...
function DelSocial(id, text) {
    ...
    DLEconfirm(text, dle_confirm, function () {
        ...
        $.get(dle_root + "engine/ajax/controller.php?mod=adminfunction", {
            id: id, action: "del_social", user_hash: dle_login_hash
        }, function (data) {
...
function subscribe(id, sub_action) {
    ...
    DLEconfirm(text, dle_confirm, function () {
        ...
        $.get(dle_root + "engine/ajax/controller.php?mod=commentssubscribe", {
            news_id: id, skin: dle_skin, sub_action: sub_action, user_hash: dle_login_hash
        }, function (data) {
...
function media_upload(area, author, news_id, wysiwyg) {
    ...
    $.get(dle_root + "engine/ajax/controller.php", {
        mod: 'upload', area: area, news_id: news_id, author: author, wysiwyg: wysiwyg
    }, function (data) {
...
function dropdownmenu(obj, e, menucontents, menuwidth) {
    ...
    if (windowx - offset.left < menudiv.width()) offset.left = offset.left - (menudiv.width() - $(obj).width());

    menudiv.css({
        left: offset.left + "px", top: offset.top + $(obj).height() + "px"
    });
...
function findCommentsPage(obj, comment_id, post_id) {
    ...
    $.post(dle_root + "engine/ajax/controller.php?mod=adminfunction", {
        action: 'findcommentspage', comment_id: comment_id, post_id: post_id, user_hash: dle_login_hash
    }, function (data) {
        HideLoading('');
...
jQuery(function ($) {
    ...
    $(document).keydown(function (event) {
        if (event.which == 13 && event.ctrlKey) {
            ...
            if (selectedText.toString().length > 255) {
                DLEalert(dle_big_text, dle_info);
                return false;
            }
    ...
    setTimeout(function () {
        $("div[data-dlebclicks]").each(function () {
            ...
            $(this).find('a').on('click', function () {
                $.post(dle_root + "engine/ajax/controller.php?mod=adminfunction", {
                    'id': id, action: 'bannersclick', user_hash: dle_login_hash
                });
            });
    ...
    if (dlebannerids.length) {
        setTimeout(function () {
            $.post(dle_root + "engine/ajax/controller.php?mod=adminfunction", {
                'ids[]': dlebannerids, action: 'bannersviews', user_hash: dle_login_hash
            });
        }, 1000);
    }
